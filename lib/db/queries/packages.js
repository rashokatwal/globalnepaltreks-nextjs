...
export const PackageQueries = {
    ...
    async findAll({ 
        ...
        let countSql = `SELECT COUNT(*) as total FROM packages WHERE is_active = ?`;
        let dataSql = `
            SELECT p.*, 
        ...
        if (search) {
            dataSql += ` AND (p.title LIKE ? OR p.short_description LIKE ? OR p.overview LIKE ?)`;
        ...
        dataSql += ` ORDER BY p.is_featured DESC, p.created_at DESC LIMIT ? OFFSET ?`;
        params.push(limit, offset);
        
        const totalResult = await query(countSql, countParams);
        const packages = await query(dataSql, params);
    ...
    // Search packages
    async search(searchTerm, { page = 1, limit = 10, countryId = null, activityId = null } = {}) {
        return await this.findAll({ 
            search: searchTerm,
            countryId,
            activityId,
