...
export const PackageQueries = {
    ...
    // Add this method to your PackageQueries - place it after the addGallery method
    async updateGallery(packageId, images, connection = null) {
        const conn = connection || { execute: (...args) => query(...args) };
        
        // First, delete all existing gallery images for this package
        await conn.execute('DELETE FROM package_gallery WHERE package_id = ?', [parseInt(packageId)]);
        ...
        if (images && images.length > 0) {
            for (let i = 0; i < images.length; i++) {
                ...
                await conn.execute(
                    ...
                    VALUES (?, ?, ?, ?)`,
                    [parseInt(packageId), img.image_url, img.title || '', i + 1]
                );
            }
        }
        return true;
