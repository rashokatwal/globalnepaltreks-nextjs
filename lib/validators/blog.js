...
export function validateBlog(data, isPartial = false) {
    ...
    if (!isPartial || data.title !== undefined) {
        ...
        else if (data.title.length > 255) errors.push('Title must be less than 255 characters');
    }
    
    if (!isPartial || data.content !== undefined) {
        if (!data.content) errors.push('Content is required');
        else if (data.content.length < 50) errors.push('Content must be at least 50 characters');
    }
